package com.hoaxify.hoaxifyspringboot.security.jwt.service;
...
public class JwtUtil {
    ...
    private final String SECRET_KEY = "devstudiodesign";
    ...
    public long tokenExpiredHours(String token) {
        ...
        return TimeUnit.MICROSECONDS.toHours(endTime - currentTime);
    }

    public long tokenExpiredSeconds(String token) {
        long endTime = extractExpiration(token).getTime();
        long currentTime = new Date().getTime();
        return TimeUnit.MICROSECONDS.toSeconds(endTime - currentTime);
    }
